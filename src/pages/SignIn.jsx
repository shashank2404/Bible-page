...
export default function SignIn() {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    // Simulate real server/authentication endpoint communication latency
    setTimeout(() => {
      setIsLoading(false);
      if (mode === "forgot") {
        setSuccess(`A password reset link has been dispatched to ${email}.`);
      } else if (mode === "signup") {
        setSuccess(`Welcome, ${name}! Your spiritual profile has been established successfully.`);
        setTimeout(() => {
          navigate("/");
        }, 2000);
      } else {
        setSuccess("Signed in successfully. Welcome back to The Bible Glory.");
        setTimeout(() => {
          navigate("/");
        }, 1500);
      }
  ...
  const handleSocialSignIn = (provider) => {
    ...
    setTimeout(() => {
      setSocialStep(`Verifying secure token signatures with ${provider}...`);
      
      setTimeout(() => {
        setSocialStep("Syncing profile credentials and establishing safe session...");
        
        setTimeout(() => {
          setIsLoading(false);
          setSuccess(`Successfully authenticated using Apple, Google, or Facebook.`);
